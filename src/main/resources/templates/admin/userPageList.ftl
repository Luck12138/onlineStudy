					...
					<div class="block-body collapse in">
						<p>
							<select  name="status" class="search-select">
								...
								<option value="2" <#if (user.status)?? && user.status ==2>selected="selected"</#if> >可用</option>
								<option value="0" <#if (user.status)?? && user.status ==0>selected="selected"</#if>>禁用</option>
							</select>
						</p>
						<p>
							<input name="username" type="text" value="${(user.username)!}" class="search-text" placeholder="请输入登录名">
						</p>
						<p>
			...
			<div class="modal-dialog " >
				<div class="modal-content">
					...
					<div class="modal-body">
						<form id="myForm" role="form" id="ff" action="/admin/userUpdate">
