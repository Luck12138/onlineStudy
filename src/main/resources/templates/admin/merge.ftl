		...
		<div class="container-fluid">
			...
			<div class="block span6">
				...
				<div class="block-body collapse in p-tb-20">
					<form role="form" method="post" action="/admin/carouselDoMerge" enctype="multipart/form-data">
						...
						<div class="form-group">
							...
							<div>
								...
								<#if (carousel.picture)?? && carousel.picture != ''>
								<img id="carousel-picture" src="${(carousel.picture)!}" style="max-width: 250px;">
								<#else>
						...
						<div class="form-group w-600">
							...
							<input type="text" class="form-control" id="name" name="name" value="${(carousel.name)!}" placeholder="请输入名称">
						</div>
						<div class="form-group  w-600">
							<label for="name">链接</label> 
							<input type="text" class="form-control" name="url" id="url" value="${(carousel.url)!}" placeholder="请输入链接">
						</div>
						<div class="form-group w-200">
							<label for="name">权重</label> 
							<input type="text" class="form-control" name="weight" id="weight" value="${(carousel.weight)!}" placeholder="请输入权重">
						</div>
						<div class="form-group w-200">
							<label for="name">是否可用</label> 
							<select name="enable" id="enable" class="form-control"  type="select">
								<option value="1" <#if (carousel.enable)?? && (carousel.enable == 1)>selected="selected"</#if>>可用</option>
								<option value="0" <#if (carousel.enable)?? && (carousel.enable == 0)>selected="selected"</#if>>禁用</option>
							</select>
						</div>
